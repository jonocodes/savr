(async function () {
  ...
  if (!savrWindow) {
    console.error("[bookmarklet] Could not open PWA window");
    alert("Could not open SAVR PWA window. Please allow pop-ups for this site.");
    return;
  }
  console.log("[bookmarklet] PWA window opened successfully");
  ...
  console.log("[bookmarklet] CSS prepared:", styleTag.textContent);
  ...
  console.log("[bookmarklet] HTML prepared, length:", htmlString.length);

  console.log("[bookmarklet] Setting up message listener...");
  var messageListener = function (event) {
    console.log("[bookmarklet] Message received:", event.data);
    if (event.source === savrWindow && event.data && event.data.action === "savr-ready") {
      console.log("[bookmarklet] PWA ready, sending HTML...");
      window.removeEventListener("message", messageListener);

      savrWindow.postMessage({ url: currentPageUrl, html: htmlString }, "*");
      console.log("[bookmarklet] HTML sent to PWA");
    }
  };

  window.addEventListener("message", messageListener);
  console.log("[bookmarklet] Message listener set up");
          ...
          try {
            ...
            if (imgElem && imgElem.complete) {
              try {
                ...
                console.log(
                  "[bookmarklet] blob size (fallback):",
                  blob.size,
                  "bytes for URL:",
                  url
                );
              ...
              const resp = await fetch(url);
              const blob = await resp.blob();
              console.log("[bookmarklet] blob size:", blob.size, "bytes for URL:", url);
              mimeType = blob.type;
              if (mimeType === "image/svg+xml") {
